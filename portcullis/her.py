...
class PrioritizedAtariBuffer(object):
    ...
    def sample(self):
        ...
        if self.prioritized:
            ...
            weights /= weights.max()
            # Hardcoded: 0.4 + 4.8e-8 * 12.5e6 = 1.0. Only used by PER.
            self.beta = min(self.beta + 4.8e-8, 1)
...
def make_env(env_name, atari_preprocessing):
    ...
    is_atari = gym.envs.registry.spec(
        env_name).entry_point == 'gym.envs.atari:AtariEnv'
