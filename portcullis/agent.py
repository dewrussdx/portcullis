...
class DDPG(object):
    ...
    class Critic(NN):
        ...
        def forward(self, state, action):
            ...
            q = torch.nn.functional.relu(self.l2(torch.cat([q, action], 1)))
            return self.l3(q)
        ...
        def __init__(self, state_dim, action_dim, max_action, discount=0.99, tau=0.001):
            ...
            self.critic = DDPG.Critic(state_dim, action_dim).to(DEVICE)
            self.critic_target = copy.deepcopy(self.critic)
            self.critic_optimizer = torch.optim.Adam(
...
class DDQN(object):
    ...
    def __init__(
        ...
        self.Q = DDQN.Conv_Q(4, num_actions).to(self.device) if is_atari else DDQN.FC_Q(
            state_dim, num_actions).to(self.device)
...
class LAP_DDQN(object):
    def __init__(
        ...
        # Determine network type
        self.Q = Conv_Q(4, num_actions).to(self.device) if is_atari else FC_Q(
            state_dim, num_actions).to(self.device)
        self.Q_target = copy.deepcopy(self.Q)
        self.Q_optimizer = getattr(torch.optim, optimizer)(
            self.Q.parameters(), **optimizer_parameters)
